...
This is a Helm charts repository. Charts are published through the repository release workflows.
...
- Run the chart-specific schema generation command when one is configured.
...
- **PR title format**: Must start with `[chart-name] ` (e.g., `[example-chart] Add new feature`).
